...
module.exports = {
    ...
    execute(message, args, prefix) {
        ...
        db.get('SELECT vouches FROM vouches WHERE user_id = ?', [vouchKey], async (err, row) => {
            ...
            const promoRoleID = '1263131220590727228'; // Replace with the actual promotion role ID
                        ...
                        if (autoRole) {
                            ...
                            if (!message.guild.members.cache.get(mentionedUser.id).roles.cache.has(autoRole.id)) {
                                try {
                                    ...
                                    if (promotionChannel && promotionChannel.isText()) {
                                        const embed = new MessageEmbed()
                                            ...
                                            .addField('Vouch Count', row.vouches.toString(), true)
                                            .setTimestamp();

                                        promotionChannel.send({ embeds: [embed] });
                                    } else {
                                        console.error(`Promotion channel not found or is not a text channel. Check your configuration.`);
                                    }
                                } catch (error) {
